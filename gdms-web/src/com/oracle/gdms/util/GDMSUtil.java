...
public final class GDMSUtil {
	...
	private static SqlSessionFactory ssf;//定义一个全局的静态代码块
	
	static {
		
		
		try { //本类被加载的时候，这个代码块只会运行一次
	...
	public static String generic(int length) {
		...
		//接着产生随机9位
		String str="ABDCDEFGHIJKLMNOPQRSTUVWSYZ0123456789_abcdefghijklmnopqrstuvwsyz";
		int count=length-s.length();
		for(int i=0;i<count;i++) {
			int n=(int) (Math.random()*str.length());//随机产生下标
			s.append(str.charAt(n)); //取出指定位置的一个字符,拼接进s中
		}
		return s.toString();
	}

	private static Object getCurrentDatetime() {
